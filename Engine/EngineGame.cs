    ...
    public virtual void DrawStrings()
    {
        foreach (UiText text in this.StringsToDraw)
        {
            ...
            if (text.ShadowColor.HasValue)
                this._spriteBatch.DrawString(text.SpriteFont, text.Text, position.Sum(1, 1), text.ShadowColor.Value, text.Rotation, Vector2.Zero, text.Scale);
            this._spriteBatch.DrawString(text.SpriteFont, text.Text, position, text.Color, text.Rotation, Vector2.Zero, text.Scale);
        }
        this.StringsToDraw.Clear();
    }

    public virtual void AddText(SpriteFontBase font, string text, Vector2 position, Color color, Vector2 origin, float rotation = 0f, Color? shadowColor = null)
    {
        this.AddText(font, text, position, color, origin, Vector2.One, rotation, shadowColor);
    }
    
    public virtual void AddText(SpriteFontBase font, string text, Vector2 position, Color color, Vector2 origin, Vector2 scale, float rotation = 0f, Color? shadowColor = null)
    {
